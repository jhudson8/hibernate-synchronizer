...
public class AddMappingReference implements IObjectActionDelegate {
    ...
    public static void addMappingReference(IProject singleProject,
            IFile[] files, boolean force, Shell shell) {
        IFile configFile = HSUtil.getConfigFile(singleProject, force, shell);
        if (null != configFile) {
            HibernateDOMParser domParser = null;
            try {
                domParser = new HibernateDOMParser(configFile.getContents());
                ...
                for (int i = 0; i < nodes.getLength(); i++) {
                    ...
                    if (subNode.getNodeName().equals("session-factory"))
                        sessionFactoryNode = subNode;
                }
                if (null != sessionFactoryNode) {
                    ...
                    for (int i = 0; i < nodes.getLength(); i++) {
                        ...
                        if (subNode.getNodeName().equals("mapping")) {
                            ...
                            if (null != mapName
                                    && mapName.getNodeValue().trim().length() > 0) {
                                existingMappings.put(mapName.getNodeValue()
                                        .trim().replace('\\', '/'), subNode);
                                lastMapping = subNode;
                            }
                        }
                    }
                    ...
                    for (int i = 0; i < files.length; i++) {
                        String directoryDifference = getDirectoryDifference(
                                configFile.getLocation().toOSString(), files[i]
                                        .getLocation().toOSString());
                        String mappingName = null;
                        if (null != directoryDifference
                                && directoryDifference.trim().length() > 0) {
                            mappingName = directoryDifference + "/"
                                    + files[i].getName();
                        } else {
                            mappingName = files[i].getName();
                        }
                        if (null == existingMappings.get(mappingName)
                                && null == existingMappings.get("/"
                                        + mappingName)) {
                            lastMapping = addMapping(mappingName,
                                    sessionFactoryNode, lastMapping, doc);
                        }
                    }
                    String contents = getDocumentContents(doc, domParser,
                            configFile.getContents());
                    configFile.delete(true, null);
                    configFile.create(new ByteArrayInputStream(contents
                            .getBytes()), true, null);
                    EditorUtil.openPage(configFile);
                } else {
                    HSUtil.showError(
                            "The session-factory node could not be located",
                            null);
                }
            } catch (SAXParseException e) {
                Plugin.clearProperty(singleProject,
                        Constants.PROP_CONFIGURATION_FILE);
                ...
                EditorUtil.addProblemMarker(configFile, e.getMessage(),
                        lineNumber);
