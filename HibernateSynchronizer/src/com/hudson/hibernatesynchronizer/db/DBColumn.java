	...
	public String getPropName (boolean checkFK) {
		...
		if (checkFK && isForeignKey()) return getFKPropName();
			...
			if (null == propName) {
				...
				if (((getPropName().toUpperCase().startsWith("IS") && getPropName().length() > 2 && Character.isUpperCase(getPropName().toCharArray()[2]))
						|| (getPropName().toUpperCase().startsWith("HAS") && getPropName().length() > 3 && Character.isUpperCase(getPropName().toCharArray()[3])))
						&& getSize() == 1) {
					int size = 2;
					if (getPropName().toUpperCase().startsWith("HAS")) size = 3;
					if (!firstLetterUpper)
